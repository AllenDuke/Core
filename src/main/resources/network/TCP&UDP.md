...
四次握手行不行？
首先四次握手是可以的，即B先发送ACK，接着再发送请求连接报文。但这两个步骤是可以合并的，即三次握手已经足够，因为建立连接不像释放连接那样，
半连接状态不能传输数据（挥手时的半连接状态可以长时间存在）。
...
## TTL (Time To Live)
TTL值表示允许路由器转发的最大次数，当达到这个最大值时，数据包就会被丢弃，TTL的默认值为1，设置为1时表明只能在子网中发送数据。
## 流量控制
...
而在java中，windows下一条channel占用2个端口。**保留意见**

linux下，select最大文件描述符数为1024

## 多个进程监听同一个端口
参考：zhihu.com/question/360939266
...
# UDP
低延迟场景下的选择。例如王者荣耀等游戏，如果选择TCP，由于可靠传输，那么在发生丢包后需重传，这样就增大了延迟。我们一般希望丢了就丢了。
流媒体场景下，似乎也一样。
...
## 多播原理
广播数据包的原理：专门用于同时向网络中所有工作站进行发送的一个地址叫做广播地址。在使用TCP/IP 协议的网络中，主机标识段host ID 为全1
的IP 地址为广播地址。如果你的IP为：192.168.1.39，子网掩码为：255.255.255.0，则广播地址为：192.168.1.255；
