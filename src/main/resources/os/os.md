...
mmap通过将磁盘文件映射到用户空间，当进程读文件时，发生缺页中断，给虚拟内存分配对应的物理内存，在通过磁盘调页操作将磁盘数据
读到物理内存上，实现了用户空间数据的读取。

## Linux内存分配机制SLAB 类似于伙伴系统
![buddy-system](../images/buddy-system.PNG)

## 进程
### 进程控制块 进程描述符 task_struct(可被预先分配和重复使用，使得可以迅速创建进程)
...
## 银行家算法（资源有序分配，避免死锁） 死锁检测与恢复
最大需求矩阵，当前需求矩阵，可用矩阵，当可用矩阵不足以分配给任何一个进程时，认为发生了死锁（用于检测），至于恢复，可选择杀死某些进程，
再次检测，或者允许抢占。
## 内存管理
### 段式 页式 段页式
段式(程序员可见)：
