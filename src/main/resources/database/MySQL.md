...
select * from t where a=1 for upda;属于当前读 insert update delete属于当前读
### 幻读的理解
...
而MySQL为了效率会打破两段锁协议，在扫描的时候发现该行记录不满足条件就会释放该行记录的锁。
#### 锁的兼容矩阵
...
### MVCC与锁
在MySQL中，如在RR下，MVCC与锁是共同起作用的。正在当前读的时候会加锁。 而加锁的时候，
读读共享，读写互斥，写写互斥，参考Java的ReentrantReadWriteLock。
### 事务与MVCC与锁
